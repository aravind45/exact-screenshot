...
const wrap = (fn: (req: any, res: Response) => Promise<any>) =>
  (req: any, res: Response) => fn(req, res).catch((e: any) => {
...
router.get('/accounting-rules', isAdmin, wrap(async (req, res) => {
  res.json(await FormsTaxSvc.listAccountingRules(req.query.jurisdictionId as string));
}));
...
router.get('/tax-obligations', isAdmin, wrap(async (req, res) => {
  res.json(await FormsTaxSvc.listTaxObligations(req.query.jurisdictionId as string));
}));
...
router.get('/distribution-rules', isAdmin, wrap(async (req, res) => {
  res.json(await FormsTaxSvc.listDistributionRules(req.query.jurisdictionId as string));
}));
...
router.get('/statute-references', isAdmin, wrap(async (req, res) => {
  ...
  res.json(await JurisdictionSvc.listStatuteReferences(entityType as string, entityId as string));
}));
...
router.get('/change-logs', isAdmin, wrap(async (req, res) => {
  ...
  res.json(await FormsTaxSvc.getChangeLogs(
    entityType as string, entityId as string, Number(limit) || 50
  ));
}));
