...
import {
    ...
    FLFieldDefinition,
    validateFLFormData,
} from './flFormRegistry.js';

export interface FLFormInput {
    formId: FLFormId;
    estate: any;
    assets?: any[];
    heirs?: any[];
...
function resolveComputed(key: string, input: FLFormInput): any {
...
async function buildFallbackFL1(fieldValues: Record<string, any>): Promise<Uint8Array> {
...
async function buildFallbackFL2(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL3(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL6(fieldValues: Record<string, any>, assets: any[]): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL7(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL10(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL11(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL14(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL15(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
export const FLFormService = {
    resolveFields(input: FLFormInput): { fieldValues: Record<string, any>; validationErrors: string[] } {
    ...
    async generate(input: FLFormInput): Promise<FLFormResult> {
        const registry = FL_FORM_REGISTRY[input.formId];
        if (!registry) {
    ...
    getUISchema(formId: FLFormId): Array<{
        key: string;
        label: string;
        type: string;
        required: boolean;
...
async function buildFallbackFL4(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL5(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL8(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL9(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL12(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackFL13(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
