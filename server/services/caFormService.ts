...
import {
    ...
    CAFieldDefinition,
    validateCAFormData,
} from './caFormRegistry.js';

// ─── Types ────────────────────────────────────────────────────────────────────

export interface CAFormInput {
    formId: CAFormId;
    estate: any;
    assets?: any[];
    heirs?: any[];
...
function resolveComputed(key: string, input: CAFormInput): any {
    ...
    switch (key) {
        ...
        case 'totalAttachment2': {
            const personalAssets = assets.filter((a: any) => a.inventoryCategory === 'ATTACHMENT_2');
...
async function buildFallbackDE111(fieldValues: Record<string, any>): Promise<Uint8Array> {
    ...
    const total = (
        Number(String(fieldValues.valuePersonalProperty || '0').replace(/,/g, '')) +
...
async function buildFallbackDE160(fieldValues: Record<string, any>, assets: any[]): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
    ...
    const realAssets = assets.filter((a: any) => a.inventoryCategory === 'ATTACHMENT_1');
    const personalAssets = assets.filter((a: any) => a.inventoryCategory === 'ATTACHMENT_2');
    const otherAssets = assets.filter((a: any) => !['ATTACHMENT_1', 'ATTACHMENT_2'].includes(a.inventoryCategory));

    const drawAssetSection = (title: string, sectionAssets: any[]) => {
        if (sectionAssets.length === 0) return;
        if (y < 100) { const np = addPage(); page = np.page; y = np.y; }
        draw(title, 11, true);
        sectionAssets.forEach((a: any) => {
            if (y < 60) { const np = addPage(); page = np.page; y = np.y; }
...
async function buildFallbackDE310(fieldValues: Record<string, any>, assets: any[]): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
export const CAFormService = {
     ...
     */
    resolveFields(input: CAFormInput): { fieldValues: Record<string, any>; validationErrors: string[] } {
     ...
     */
    async generate(input: CAFormInput): Promise<CAFormResult> {
        const registry = CA_FORM_REGISTRY[input.formId];
        if (!registry) {
     ...
     */
    getUISchema(formId: CAFormId): Array<{
        key: string;
        label: string;
        type: string;
        required: boolean;
