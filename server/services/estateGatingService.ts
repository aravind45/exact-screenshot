...
import {
    ...
    type AuthorityType,
    type EstateGatesResolution
} from "../middleware/authorityGating.js";
...
export const EstateGatingService = {
     ...
     */
    async getGateResolution(estateId: string): Promise<EstateGatesResolution> {
    ...
    async checkOperation(
        ...
        message: string;
        resolution: EstateGatesResolution;
    }> {
     ...
     */
    async getAuthoritySummary(estateId: string): Promise<{
        estateId: string;
        currentPhase: string;
        ...
        canHandleCreditors: boolean;
        canSellRealEstate: boolean;
     ...
     */
    async canUpgradeAuthority(estateId: string): Promise<{
        canUpgrade: boolean;
        currentType: AuthorityType;
        possibleUpgrades: AuthorityType[];
        ...
        const currentType = (estate.authorityType || "UNSET") as AuthorityType;
        const possibleUpgrades: AuthorityType[] = [];
     ...
     */
    async batchCheckGates(
        estateIds: string[],
        gate: GateType
    ): Promise<Record<string, { allowed: boolean; message: string }>> {
        const results: Record<string, { allowed: boolean; message: string }> = {};
     ...
     */
    async getAvailableOperations(estateId: string): Promise<{
        operation: string;
        allowed: boolean;
        requiredGates: GateType[];
    }[]> {
        const resolution = await resolveEstateGates(estateId);
        const operations: { operation: string; allowed: boolean; requiredGates: GateType[] }[] = [];
     ...
     */
    getEnforcementMatrix(): typeof ENFORCEMENT_MATRIX {
