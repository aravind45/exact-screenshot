...
function resolveComputed(key: string, input: TXFormInput): any {
...
async function buildFallbackTX1(fieldValues: Record<string, any>): Promise<Uint8Array> {
...
async function buildFallbackTX2(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackTX3(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackTX7(fieldValues: Record<string, any>, assets: any[]): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackTX9(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackTX11(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackTX12(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
export const TXFormService = {
    resolveFields(input: TXFormInput): { fieldValues: Record<string, any>; validationErrors: string[] } {
    ...
    async generate(input: TXFormInput): Promise<TXFormResult> {
        const registry = TX_FORM_REGISTRY[input.formId];
        if (!registry) {
    ...
    getUISchema(formId: TXFormId): Array<{
        key: string;
        label: string;
        type: string;
        required: boolean;
...
async function buildFallbackTX4(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackTX5(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackTX6(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackTX8(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
...
async function buildFallbackTX10(fieldValues: Record<string, any>): Promise<Uint8Array> {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
