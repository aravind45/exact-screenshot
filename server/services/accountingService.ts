...
export class AccountingService {
     ...
     */
    static async getReadiness(estateId: string): Promise<AccountingReadiness> {
        ...
        if (!inventoryObtained) details.push("Final Inventory & Appraisal not yet filed.");
        if (!assetsVerified) details.push("Some discovered assets have not been verified.");
        if (!noticePeriodClosed) details.push("Creditor notice period is still open.");
        if (!claimsResolved) details.push("There are unpaid or unresolved liabilities.");

        let status: 'DRAFT' | 'READY_FOR_REVIEW' | 'INCOMPLETE' = 'INCOMPLETE';
