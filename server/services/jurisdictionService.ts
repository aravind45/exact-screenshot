...
/**
 * Jurisdiction configuration for each state
 */
export interface JurisdictionConfig {
  code: string;
  name: string;
  probateCourt: string;
  surrogateCourt: string;
  smallEstateThreshold: number;
  smallEstateTerm: string;
  probateTerm: string;
  lettersTerm: string;
  isUPC: boolean;
  citationStyle: "bluebook" | "alwd" | "state";
  formServiceAvailable: boolean;
  priorityServiceAvailable: boolean;
}

/**
 * Citation with statute reference
 */
...
/**
 * Task binding to statute
 */
export interface TaskStatuteBinding {
  taskId: string;
  stateCode: string;
 ...
 */
export function createJurisdictionConfig(stateCode: string): JurisdictionConfig {
  const stateRule = getStateRule(stateCode);
  const stateInfo = US_STATES.find(s => s.abbr === stateCode);
  
  const formServiceMap: Record<string, boolean> = {
    "CA": true,
    "FL": true,
    "NJ": true,
    "NY": true,
  ...
  const priorityServiceMap: Record<string, boolean> = {
    "CA": true,
    "FL": true,
    "NJ": true,
    "NY": true,
 ...
 */
function getProbateCourtName(stateCode: string): string {
 ...
 */
function getSurrogateCourtName(stateCode: string): string {
  const surrogateCourts: Record<string, string> = {
    "NY": "Surrogate's Court",
    "CT": "Probate Court",
    "MD": "Orphan's Court",
    "PA": "Orphan's Court",
 ...
 */
function getCitationStyle(stateCode: string): "bluebook" | "alwd" | "state" {
  // New York uses its own style
  if (stateCode === "NY") return "state";
...
export const JURISDICTION_REGISTRY: Record<string, JurisdictionConfig> = 
  Object.fromEntries(
    ALL_JURISDICTIONS.map(code => [code, createJurisdictionConfig(code)])
  );
...
export const StatuteBindingService = {
   ...
   */
  getBindingsForState(stateCode: string): TaskStatuteBinding[] {
   ...
   */
  validateBinding(taskId: string, stateCode: string): {
    isValid: boolean;
    warnings: string[];
    errors: string[];
  } {
   ...
   */
  getCitationsForTask(taskId: string, stateCode: string): StatuteCitation[] {
    const stateRule = getStateRule(stateCode);
    
    // Map task IDs to their statutory citations
    const taskCitationMap: Record<string, StatuteCitation[]> = {
...
export const JurisdictionValidator = {
   ...
   */
  isValidJurisdiction(stateCode: string): boolean {
   ...
   */
  getConfig(stateCode: string): JurisdictionConfig | undefined {
   ...
   */
  validateEstate(stateCode: string, county?: string): {
    isValid: boolean;
    warnings: string[];
    errors: string[];
  } {
...
export const CitationFormatter = {
   ...
   */
  format(citation: string, stateCode: string): string {
    const config = JURISDICTION_REGISTRY[stateCode];
    if (!config) return citation;
   ...
   */
  formatBluebook(citation: string): string {
   ...
   */
  formatStateStyle(citation: string, stateCode: string): string {
   ...
   */
  parseCitation(citation: string): { code: string; section: string } | null {
 ...
 */
export function getValidatedStateRule(stateCode: string): StateRule {
