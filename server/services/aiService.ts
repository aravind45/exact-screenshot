...
// Types
interface Pattern {
    type: 'INSTITUTION' | 'ACCOUNT_NUMBER' | 'MASKED_ACCOUNT' | 'AMOUNT';
    subtype?: string;
    name?: string;
    value?: string | number;
    position: number;
...
export interface DiscoveredAsset {
    ...
    asset: {
        name: string;
        institution: string;
        assetType: 'CHECKING' | 'SAVINGS' | 'INVESTMENT' | 'PROPERTY' | 'VEHICLE' | 'OTHER';
        value?: number;
...
export const AiService = {
     ...
     */
    async analyzeDocument(filePath: string): Promise<DiscoveredAsset[]> {
    ...
    async detectPatterns(text: string): Promise<Pattern[]> {
    ...
    async findCorrelations(patterns: Pattern[], entities: Entity[]): Promise<Correlation[]> {
        ...
        institutions.forEach(inst => {
            ...
            const nearbyAccounts = accounts.filter(acc =>
                Math.abs(acc.position - inst.position) < 300
            ).sort((a, b) =>
                Math.abs(a.position - inst.position) - Math.abs(b.position - inst.position)
            );

            // Find nearest amount (within 300 characters)
            const nearbyAmounts = amounts.filter(amt =>
                Math.abs(amt.position - inst.position) < 300
            ).sort((a, b) =>
                Math.abs(a.position - inst.position) - Math.abs(b.position - inst.position)
            );
            ...
            if (nearbyAccounts.length > 0 || nearbyAmounts.length > 0) {
                correlations.push({
                    ...
                    confidence: Math.min(
                        inst.confidence,
                        Math.max(
                            nearbyAccounts[0]?.confidence || 0.7,
                            nearbyAmounts[0]?.confidence || 0.7
                        )
                    )
                });
            }
    ...
    generateFindings(correlations: Correlation[]): DiscoveredAsset[] {
        const findings: DiscoveredAsset[] = [];

        correlations.forEach(corr => {
            if (corr.institution) {
                const finding: DiscoveredAsset = {
                    ...
                    asset: {
                        ...
                        accountNumber: corr.accounts?.[0]?.value as string,
                        value: corr.amounts?.[0]?.value as number
                    },
    ...
    buildSourceText(corr: Correlation): string {
        ...
        if (corr.amounts && corr.amounts.length > 0) {
            text += ` showing balance of $${(corr.amounts[0].value as number).toLocaleString()}`;
        }
        return text;
    },

    mapToAssetType(subtype: string): 'CHECKING' | 'SAVINGS' | 'INVESTMENT' | 'PROPERTY' | 'VEHICLE' | 'OTHER' {
    ...
    getEducationalNote(type: string): string {
