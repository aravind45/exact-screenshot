...
// ─── Legal Forms ───
export async function listLegalForms(jurisdictionId?: string) {
...
export async function getLegalForm(id: string) {
  const rows = await queryRows(`SELECT * FROM ssot_legal_forms WHERE id=$1`, id);
  return rows[0] || null;
}

export async function upsertLegalForm(data: any, userId?: string) {
  if (data.id) {
    await executeSQL(`
      ...
      WHERE id=$1
    `, data.id, data.formNumber, data.title, data.description||null, data.category||null,
      data.jurisdictionId||null, data.isUniversal||false, data.filingMethod||'PHYSICAL',
      data.filingFeeAmount||null, data.externalUrl||null, data.officialSourceUrl||null,
      data.sourceCitation||null, data.effectiveDate||null);
  ...
  `, data.formNumber, data.title, data.description||null, data.category||null,
    data.jurisdictionId||null, data.isUniversal||false, data.filingMethod||'PHYSICAL',
    data.filingFeeAmount||null, data.externalUrl||null, data.officialSourceUrl||null,
    data.sourceCitation||null, data.effectiveDate||null);
  const c = rows[0];
...
export async function deleteLegalForm(id: string, userId?: string) {
...
export async function upsertAssetType(data: any, userId?: string) {
  if (data.id) {
    await executeSQL(`
      ...
      WHERE id=$1
    `, data.id, data.code, data.name, data.category, data.description||null,
      data.probateInclusionDefault||'INCLUDED', data.requiresAppraisal||false,
      data.typicalTransferMethod||null);
...
export async function upsertLiabilityType(data: any, userId?: string) {
  if (data.id) {
    await executeSQL(`
      ...
      WHERE id=$1
    `, data.id, data.code, data.name, data.description||null,
      data.priorityClass||'GENERAL', data.priorityRank||99);
    return { id: data.id, ...data };
  }
  const rows = await queryRows(`
...
export async function deleteLiabilityType(id: string) {
  await executeSQL(`DELETE FROM ssot_liability_types WHERE id=$1`, id);
}

// ─── Accounting Rules ───
export async function listAccountingRules(jurisdictionId?: string) {
...
export async function upsertAccountingRule(data: any, userId?: string) {
  if (data.id) {
    await executeSQL(`
      UPDATE ssot_accounting_rules SET jurisdiction_id=$2, rule_type=$3, code=$4,
        ...
        version=version+1, updated_at=NOW()
      WHERE id=$1
    `, data.id, data.jurisdictionId||null, data.ruleType, data.code, data.title,
      data.description||null, data.requirementLevel||'REQUIRED', data.frequency||null,
      data.deadlineRule||null, data.deadlineDays||null, data.sourceCitation||null);
    return { id: data.id, ...data };
  }
  const rows = await queryRows(`
  ...
  `, data.jurisdictionId||null, data.ruleType, data.code, data.title,
    data.description||null, data.requirementLevel||'REQUIRED', data.frequency||null,
    data.deadlineRule||null, data.deadlineDays||null, data.sourceCitation||null);
  return rows[0];
}
...
// ─── Tax Obligations ───
export async function listTaxObligations(jurisdictionId?: string) {
...
export async function upsertTaxObligation(data: any, userId?: string) {
  if (data.id) {
    await executeSQL(`
      UPDATE ssot_tax_obligations SET jurisdiction_id=$2, tax_type=$3, code=$4,
        ...
        version=version+1, updated_at=NOW()
      WHERE id=$1
    `, data.id, data.jurisdictionId||null, data.taxType, data.code, data.title,
      data.description||null, data.isFederal||false, data.appliesToEstatesAbove||null,
      data.taxRateInfo||null, data.filingFormNumber||null, data.filingDeadlineRule||null,
      data.filingDeadlineDays||null, data.irsFormUrl||null, data.sourceCitation||null);
    return { id: data.id, ...data };
  }
  const rows = await queryRows(`
  ...
  `, data.jurisdictionId||null, data.taxType, data.code, data.title,
    data.description||null, data.isFederal||false, data.appliesToEstatesAbove||null,
    data.taxRateInfo||null, data.filingFormNumber||null, data.filingDeadlineRule||null,
    data.filingDeadlineDays||null, data.irsFormUrl||null, data.sourceCitation||null);
  return rows[0];
...
// ─── Distribution Rules ───
export async function listDistributionRules(jurisdictionId?: string) {
  const where = jurisdictionId ? `WHERE jurisdiction_id = $1 OR jurisdiction_id IS NULL` : '';
  const params = jurisdictionId ? [jurisdictionId] : [];
  return queryRows(`SELECT * FROM ssot_distribution_rules ${where} ORDER BY priority_order NULLS LAST, code`, ...params);
}

export async function upsertDistributionRule(data: any, userId?: string) {
  if (data.id) {
    await executeSQL(`
      ...
      WHERE id=$1
    `, data.id, data.jurisdictionId||null, data.code, data.title, data.description||null,
      data.ruleType||'INTESTACY', data.conditionLogic ? JSON.stringify(data.conditionLogic) : null,
      data.shareFormula||null, data.priorityOrder||null, data.sourceCitation||null);
    return { id: data.id, ...data };
  }
  const rows = await queryRows(`
  ...
  `, data.jurisdictionId||null, data.code, data.title, data.description||null,
    data.ruleType||'INTESTACY', data.conditionLogic ? JSON.stringify(data.conditionLogic) : null,
    data.shareFormula||null, data.priorityOrder||null, data.sourceCitation||null);
  return rows[0];
}
...
// ─── Change Logs ───
export async function getChangeLogs(entityType?: string, entityId?: string, limit = 50) {
