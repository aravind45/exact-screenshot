...
export async function runFullGapAnalysis(): Promise<GapReport> {
  const gaps: GapItem[] = [];

  // 1. States missing jurisdictions
  const existingJurisdictions = await queryRows(`SELECT state_code FROM ssot_jurisdictions`);
  const existingCodes = new Set(existingJurisdictions.map((j: any) => j.state_code));
...
export async function getStateCompleteness() {
  ...
  const missingStates = ALL_50_STATES.filter(
    sc => !jurisdictions.find((j: any) => j.state_code === sc)
  );
