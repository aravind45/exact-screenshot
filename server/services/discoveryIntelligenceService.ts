...
export class DiscoveryIntelligenceService {
     ...
     */
    static async getDiscoveryInsights(estateId: string): Promise<DiscoveryInsight[]> {
        const documents = await prisma.estateDocument.findMany({
            where: { estateId, clues: { not: null } }
        });

        const allClues: DiscoveryClue[] = [];
        documents.forEach(doc => {
            const clues = doc.clues as any;
            if (Array.isArray(clues)) {
                allClues.push(...clues);
            } else if (clues && typeof clues === 'object') {
                // Handle different clue formats if necessary
                if (Array.isArray(clues.clues)) allClues.push(...clues.clues);
                else if (clues.data) allClues.push(clues.data);
            }
        });
     ...
     */
    static async correlateFindings(estateId: string, currentFindings: any[]) {
        ...
        const newLeads = currentFindings.filter(finding => {
            const isDuplicate = existingAssets.some(asset =>
                asset.institution.toLowerCase().includes(finding.asset.institution.toLowerCase()) ||
