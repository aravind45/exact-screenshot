...
export interface AuthorityRecommendationResult {
  ...
  changeReason: string;
  changeSource: AuthorityChangeSource;
  requiresRepin: boolean;
  authorityChangePending: boolean;
}

export interface RepinPreviewResult {
  currentAuthorityType: string;
  recommendedAuthorityType: string;
  estateAuthorityType: EstateAuthorityType;
  recommendedEstateAuthorityType: EstateAuthorityType;
...
export async function computeAuthorityRecommendation(estateId: string): Promise<AuthorityRecommendationResult> {
  ...
  const currentEstateAuthorityType = (estate.estateAuthorityType as EstateAuthorityType) || deriveEstateAuthorityType(rec.activeEngines);
...
export async function getRepinPreview(estateId: string): Promise<RepinPreviewResult> {
  ...
  const currentEstateAuthorityType = (estate.estateAuthorityType as EstateAuthorityType) || "BOTH";
  ...
  const addedWithTitles: { id: string; title: string }[] = [];
  const removedWithTitles: { id: string; title: string }[] = [];
 ...
 */
async function getTasksForAuthorityType(stateCode: string, estateAuthorityType: EstateAuthorityType): Promise<PhaseTask[]> {
...
export async function updateEstateAuthorityType(params: {
  estateId: string;
  newAuthorityType: string;
  newEstateAuthorityType: EstateAuthorityType;
  changeSource: AuthorityChangeSource;
  triggeredBy?: string;
  pinAfterUpdate?: boolean;
}): Promise<void> {
  const estate = await prisma.estate.findUnique({
 ...
 */
export async function pinAuthorityType(
  estateId: string,
  userId: string
): Promise<{ success: boolean; authorityType: string; estateAuthorityType: EstateAuthorityType; pinnedAt: Date }> {
  const recommendation = await computeAuthorityRecommendation(estateId);
  ...
  return {
    ...
    estateAuthorityType: (estate?.estateAuthorityType as EstateAuthorityType) || recommendation.recommendedEstateAuthorityType,
 ...
 */
export async function repinAuthorityType(
  estateId: string,
  userId: string,
  confirm: boolean = false
): Promise<{ success: boolean; requiresConfirmation?: boolean; preview?: RepinPreviewResult }> {
 ...
 */
export async function getAuthorityChangeHistory(estateId: string): Promise<any[]> {
 ...
 */
export async function revertAuthorityChange(
  estateId: string,
  userId: string,
  reason: string
