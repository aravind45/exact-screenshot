 ...
 */
export const ALL_INTEGRITY_CHECKS: Array<{
  id: string;
  name: string;
  description: string;
 ...
 */
function convertViolationsToFindings(
  checkId: string,
  checkName: string,
  stateCode: string,
  violations: Array<{
  ...
  return violations.map(v => {
    ...
    else if (v.severity === 'WARNING') severity = 'WARNING';
...
export async function runIntegrityCheck(
  ...
  try {
    ...
    const severityCounts = findings.reduce(
      ...
      { BLOCKER: 0, CRITICAL: 0, WARNING: 0, INFO: 0 } as Record<FindingSeverity, number>
    );
  ...
  for (const checkDef of checksToRun) {
    ...
    try {
      ...
      const result = await Promise.race([
        ...
        new Promise<IntegrityCheckResult>((_, reject) =>
          setTimeout(() => reject(new Error(`Check "${checkDef.id}" timed out after ${timeoutMs}ms`)), timeoutMs)
        ),
      ]);
 ...
 */
export function calculateOverallStatus(results: IntegrityCheckResult[]): 'PASSED' | 'FAILED' {
 ...
 */
export function aggregateFindings(results: IntegrityCheckResult[]) {
 ...
 */
export function getIntegrityCheck(checkId: string): IntegrityCheck | null {
  const checkDef = ALL_INTEGRITY_CHECKS.find(c => c.id === checkId);
