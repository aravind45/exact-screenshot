/**
 * Cross-State Statute Leak Integrity Check
 * 
...
const STATE_STATUTE_PATTERNS: Record<string, RegExp[]> = {
  ...
  // Kansas
  'KS': [/\bK\.?\s*S\.?\s*A\.?/i, /\bKansas\sStat\.?\s*Ann\.?/i],
  // Utah
 ...
 */
function getOtherStatePatterns(stateCode: string): Array<{ state: string; patterns: RegExp[] }> {
 ...
 */
function scanForLeaks(text: string, stateCode: string): Array<{ foundState: string; pattern: string; match: string }> {
  const leaks: Array<{ foundState: string; pattern: string; match: string }> = [];
 ...
 */
export function validateCrossStateStatuteLeak(
  stateCode: string,
  ...
  for (const task of tasks) {
    ...
    if (task.title) {
      ...
      for (const leak of titleLeaks) {
        findings.push({
          checkId: 'crossStateStatuteLeak',
          checkName: 'Cross-State Statute Leak',
          stateCode,
          severity: 'CRITICAL',
          ...
          remediation: {
            ...
            steps: [
              ...
              `Replace ${leak.foundState} statute citation with ${stateCode} equivalent`,
              `Verify the replacement is legally accurate`,
            ],
          },
        });
      }
      ...
      for (const leak of overrideLeaks) {
        findings.push({
          ...
          remediation: {
            type: 'manual',
            steps: [
              `Review state override for task "${task.title}" in the codebase`,
              `Remove ${leak.foundState} statute citation from ${stateCode} override`,
              `Replace with ${stateCode} equivalent if needed`,
            ],
          },
        });
      }
