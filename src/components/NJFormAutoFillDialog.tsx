...
interface NJFormAutoFillDialogProps {
    open: boolean;
...
const NJ_FORM_DESCRIPTIONS: Record<string, string> = {
    'NJ-1': "Application for Probate. Initiates the process in the Surrogate's Court. Auto-filled with decedent and petitioner data.",
    'NJ-2': "Authorization to Accept Service of Process. Required for the Surrogate to handle certain legal notifications.",
};

export function NJFormAutoFillDialog({ open, onOpenChange, formId, formTitle }: NJFormAutoFillDialogProps) {
    const [overrides, setOverrides] = useState<Record<string, any>>({});
    const [showAllFields, setShowAllFields] = useState(false);

    const { data: schemaData, isLoading: schemaLoading } = useQuery({
        queryKey: ['nj-form-schema', formId],
        queryFn: () => api.getNJFormSchema(formId),
        enabled: open && !!formId,
    });

    const { data: previewData, isLoading: previewLoading } = useQuery({
        queryKey: ['nj-form-preview', formId, overrides],
        queryFn: () => api.previewNJFormFields(formId, overrides),
        enabled: open && !!formId,
        staleTime: 0,
    });
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                                <Badge variant="outline" className="text-[9px] font-black uppercase tracking-widest border-indigo-300 text-indigo-700 px-2 py-0.5">
                                    {formId}
                                </Badge>
                            ...
                            <DialogDescription className="text-xs text-gray-500 mt-1 leading-relaxed">
                                {NJ_FORM_DESCRIPTIONS[formId] || 'Generate a pre-filled draft using your estate data.'}
                            </DialogDescription>
                        </div>
                    </div>
                </DialogHeader>
                            ...
                            <>
                                    ...
                                    {previewLoading ? (
                                        ...
                                        >
                                            ...
                                            <span className="text-xs font-bold text-emerald-700 uppercase tracking-wider">
                                                Ready — all required fields found
                                            </span>
                                        </motion.div>
                                    ) : (
                                        <motion.div
                                ...
                                <div className="flex items-start gap-2 p-3 bg-blue-50 rounded-xl border border-blue-100">
                                    ...
                                    <p className="text-[10px] text-blue-700 leading-relaxed">
                                        This generates a structured draft for review. Official NJ forms vary by county surrogate.
                                    </p>
                                </div>
                            </>
                        )}
