...
export function ProbateChecklistWidget({ estateType, deceasedState = "" }: ProbateChecklistWidgetProps) {
    ...
    const dynamicStages = useMemo(() => {
        ...
        return generateRoadmap(recommendation.type, estate?.deceasedState || deceasedState, modifiers, recommendation.activeEngines, estate?.hasWill);
    }, [recommendation, estate?.deceasedState, deceasedState, estate?.isInternational, estate?.hasWill]);
            ...
            <Card className="border-slate-200 shadow-sm bg-indigo-50/30 border-indigo-100 overflow-hidden">
                <CardContent className="p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-4 text-left">
                        ...
                        <div>
                            <h3 className="text-xs font-black text-indigo-900 uppercase tracking-tight">Finalizing Your Action Plan</h3>
