    ...
    if (pendingTasks.length === 0 && pendingAssets.length === 0) {
        return (
            <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl text-center">
                ...
                <p className="text-[10px] text-emerald-700 mt-0.5 uppercase font-bold tracking-widest">Ready for Review</p>
            </div>
        );
    }
                    ...
                    <div>
                        ...
                        <div className="space-y-1.5">
                            {pendingAssets.slice(0, 3).map(asset => (
                                <div key={asset.id} className="p-2 bg-indigo-50/50 border border-indigo-100/50 rounded-xl flex items-start gap-2 hover:bg-indigo-50 transition-colors cursor-pointer" onClick={() => navigate(`/asset/${asset.id}`)}>
