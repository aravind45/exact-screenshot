...
interface CAFormAutoFillDialogProps {
    open: boolean;
...
const CA_FORM_DESCRIPTIONS: Record<string, string> = {
    'DE-111': 'Initiates the probate process. Auto-filled with decedent information, petitioner details, and estimated estate values.',
    'DE-160': 'Documents all estate assets for the court. Auto-filled from your asset inventory with real and personal property totals.',
    'DE-310': 'Petitions the court to determine succession to real property. Requires a legal property description.',
};

const INVENTORY_TYPE_OPTIONS = [
    { value: 'FINAL', label: 'Final Inventory' },
    { value: 'PARTIAL', label: 'Partial Inventory (more assets pending)' },
    { value: 'SUPPLEMENTAL', label: 'Supplemental (assets found after filing)' },
];

export function CAFormAutoFillDialog({ open, onOpenChange, formId, formTitle }: CAFormAutoFillDialogProps) {
    const [overrides, setOverrides] = useState<Record<string, any>>({});
    const [showAllFields, setShowAllFields] = useState(false);

    const { data: schemaData, isLoading: schemaLoading } = useQuery({
        queryKey: ['ca-form-schema', formId],
        queryFn: () => api.getCAFormSchema(formId),
        enabled: open && !!formId,
    });
    ...
    const renderFieldInput = (field: FieldSchema) => {
        const currentValue = overrides[field.key] ?? fieldValues[field.key] ?? '';

        if (field.key === 'inventoryType') {
            return (
                <Select
                    value={String(currentValue || 'FINAL')}
                    onValueChange={val => handleFieldChange(field.key, val)}
                >
                    <SelectTrigger className="h-9 text-sm border-gray-200 focus:border-primary/50">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <DialogDescription className="text-xs text-gray-500 mt-1 leading-relaxed">
                                {CA_FORM_DESCRIPTIONS[formId] || 'Generate a pre-filled draft using your estate data.'}
                            </DialogDescription>
                        </div>
                    </div>
                </DialogHeader>
                    ...
                    <div className="px-6 py-5 space-y-5">
                        {schemaLoading ? (
                            ...
                            <>
                                ...
                                {/* Auto-filled field values preview */}
                                ...
                                <div className="flex items-start gap-2 p-3 bg-blue-50 rounded-xl border border-blue-100">
                                    ...
                                    <p className="text-[10px] text-blue-700 leading-relaxed">
                                        This generates a structured draft for attorney review. Upload the official Judicial Council
