...
interface TXFormAutoFillDialogProps {
    open: boolean;
...
const TX_FORM_DESCRIPTIONS: Record<string, string> = {
    'TX-1': 'Application for probate of will in Texas county court. Auto-filled with applicant, decedent, and county details.',
    'TX-2': 'Application for letters of administration when there is no will. Includes petitioner and heir-facing intake data.',
    'TX-3': 'Application for independent administration. Uses Texas-specific administration type data from your estate profile.',
    'TX-7': 'Inventory, appraisement, and list of claims. Auto-filled from your estate assets and liabilities.',
    'TX-8': 'Annual account. Requires accounting-period details that can be entered as overrides before download.',
    'TX-9': 'Application to close the estate. Uses current estate totals and lets you supply the distribution plan.',
    'TX-12': 'Muniment of title petition. Requires a property description override for the final filing draft.',
};

export function TXFormAutoFillDialog({ open, onOpenChange, formId, formTitle }: TXFormAutoFillDialogProps) {
    const [overrides, setOverrides] = useState<Record<string, any>>({});
    const [showAllFields, setShowAllFields] = useState(false);

    const { data: schemaData, isLoading: schemaLoading } = useQuery({
        queryKey: ['tx-form-schema', formId],
        queryFn: () => api.getTXFormSchema(formId),
        enabled: open && !!formId,
    });

    const { data: previewData, isLoading: previewLoading } = useQuery({
        queryKey: ['tx-form-preview', formId, overrides],
        queryFn: () => api.previewTXFormFields(formId, overrides),
        enabled: open && !!formId,
        staleTime: 0,
    });
    ...
    const generateMutation = useMutation({
        mutationFn: async ({ isPreview }: { isPreview: boolean }) => {
            const blob = await api.generateTXForm(formId, isPreview, overrides);
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <DialogDescription className="text-xs text-gray-500 mt-1 leading-relaxed">
                                {TX_FORM_DESCRIPTIONS[formId] || 'Generate a pre-filled draft using your estate data.'}
                            </DialogDescription>
                        </div>
                    </div>
                </DialogHeader>
