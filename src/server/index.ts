...
app.get("/api/tenant-config", (req, res) => {
    ...
    let tenant;
    if (tenantParam && TENANTS[tenantParam as keyof typeof TENANTS]) {
        tenant = TENANTS[tenantParam as keyof typeof TENANTS];
    } else {
...
app.use("/api/public", publicStatusRoutes); // token-guarded, no session auth — see publicStatusRoutes.ts
...
app.put("/api/auth/me", authenticate, async (req: any, res) => {
    try {
        ...
        // NOTE: Profile state = executor's personal state (where they live).
        // Deceased's state is stored on estate.deceasedState and set via onboarding / estate settings.
        // These are intentionally separate — an executor in TX may settle an estate in NY.
