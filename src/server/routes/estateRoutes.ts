...
import { generateStatusToken } from "./publicStatusRoutes.js";
...
router.get("/", async (req: any, res: Response) => {
    try {
        const estates = await prisma.estate.findMany({
            ...
            // stripeCustomerId) — field whitelist only
            include: { user: { select: { id: true, fullName: true, email: true, personalEmail: true } } },
...
/**
 * GET /my/status-link
 * Returns the executor's shareable "family update" link — a read-only,
 * token-guarded status page they can text to relatives instead of answering
 * "where's the money?" every week. No schema changes: the token is an HMAC
 * of the estate ID (see publicStatusRoutes.ts).
 */
router.get("/my/status-link", async (req: any, res: Response) => {
    try {
        ...
        const token = generateStatusToken(estate.id);
        const appUrl = (process.env.APP_URL || 'http://localhost:5173').replace(/\/+$/, '');
        res.json({ url: `${appUrl}/estate-status/${estate.id}/${token}` });
    } catch (error: any) {
        logger.error("Error generating status link:", error.message);
    ...
    try {
        ...
        try {
            estate = await prisma.estate.findFirst({
                ...
                // NEVER serialize credential fields — field whitelist only
                include: { user: { select: { id: true, fullName: true, email: true, personalEmail: true } }, heirs: true }
            });
        } catch (dbError: any) {
