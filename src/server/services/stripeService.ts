...
const ESTATE_LICENSE_PRICE_ID = process.env.STRIPE_ESTATE_LICENSE_PRICE_ID || 'price_estate_license_placeholder'; // $299 one-time Estate License
...
export class StripeService {
    ...
    static async createCheckoutSession(userId: string, successUrl: string, cancelUrl: string, skipTrial = false) {
        ...
        const session = await this.stripe.checkout.sessions.create({
            ...
            subscription_data: skipTrial ? undefined : {
                trial_period_days: TRIAL_DAYS, // shared constant from utils/trialUtils — matches subscription middleware + billing routes
            },
     ...
     * One payment covers the estate for its entire settlement — matches how
     * executors actually think (compare: EstateExec one-time pricing) and
     * removes the churn trigger of paying monthly through creditor waiting
     * periods. Webhook (type ESTATE_LICENSE) marks the account ACTIVE.
     */
    static async createEstateLicenseCheckoutSession(userId: string) {
        const user = await prisma.user.findUnique({ where: { id: userId } });
        if (!user) throw new Error('User not found');
    ...
    static async handleWebhook(event: Stripe.Event) {
        ...
        switch (event.type) {
            case 'checkout.session.completed': {
                ...
                // Handle Estate License (one-time payment → permanent ACTIVE)
                if (session.metadata?.type === 'ESTATE_LICENSE') {
                    await prisma.user.update({
                        where: { id: userId },
                    ...
                    await prisma.transaction.create({
                        data: {
                            ...
                            stripePaymentIntentId: session.payment_intent as string,
                            type: 'PAYMENT',
                            notes: 'Estate License (one-time) activated',
                        },
                    });

                    logger.info(`✅ Estate License activated for user ${userId}`);
                }
