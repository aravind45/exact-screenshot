...
const ADVISOR_TYPES: { value: AdvisorType; label: string }[] = [
  ...
  { value: 'COACH', label: 'Estate Coach' },
];
...
const normalizeMarketplaceAdvisor = (advisor: any): MarketplaceAdvisor => ({
  id: advisor?.id || '',
  bio: advisor?.bio || '',
  advisorType: advisor?.advisorType || 'ATTORNEY',
  specialties: toStringArray(advisor?.specialties ?? advisor?.expertise),
  statesServed: toStringArray(advisor?.statesServed),
  hourlyRate: Number(advisor?.hourlyRate || 0),
  averageRating: Number(advisor?.averageRating ?? advisor?.avgRating ?? 0),
  totalReviews: Number(advisor?.totalReviews || 0),
  verificationStatus: advisor?.verificationStatus || advisor?.status || 'PENDING',
  user: {
...
function AdvisorCard({ advisor }: { advisor: MarketplaceAdvisor }) {
  ...
  const fullName = advisor.user?.fullName || 'Unknown Advisor';
        ...
        <CardHeader className="pb-3">
          <div className="flex items-start gap-3">
            <div className="w-14 h-14 rounded-xl bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-lg flex-shrink-0 overflow-hidden">
              {advisor.profileImage ? <img src={advisor.profileImage} alt={fullName} className="w-full h-full object-cover" /> : <span>{initials}</span>}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                <CardTitle className="text-base font-bold truncate">{fullName}</CardTitle>
        ...
        <CardContent className="flex-1 space-y-3 pt-0">
          ...
          <div className="flex flex-wrap gap-1">
            {(advisor.specialties ?? []).slice(0, 3).map((specialty) => (
              <Badge key={specialty} variant="secondary" className="text-xs bg-slate-100 text-slate-600 border-0">
                {SPECIALTIES.find((sp) => sp.value === specialty)?.label ?? specialty}
              </Badge>
            ))}
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-2">
          {SPECIALTIES.map((specialty) => (
            <div key={specialty.value} className="flex items-center gap-2">
              <Checkbox id={`spec-${specialty.value}`} checked={filters.specialties.includes(specialty.value)} onCheckedChange={() => toggle(specialty.value)} />
              <Label htmlFor={`spec-${specialty.value}`} className="text-sm text-slate-600 cursor-pointer">{specialty.label}</Label>
            </div>
          ))}
        </div>
      </div>
      <div>
        ...
        <Select value={filters.state || 'ALL'} onValueChange={(value) => onFiltersChange({ ...filters, state: value === 'ALL' ? '' : value })}>
      ...
      <div>
        ...
        <Select value={filters.advisorType || 'ALL'} onValueChange={(value) => onFiltersChange({ ...filters, advisorType: value === 'ALL' ? '' : value })}>
          <SelectTrigger className="w-full"><SelectValue placeholder="All Types" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="ALL">All Types</SelectItem>
            {ADVISOR_TYPES.map((type) => <SelectItem key={type.value} value={type.value}>{type.label}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      <div>
        ...
        <div className="px-1 space-y-4">
          <div><Label className="text-xs text-slate-500 mb-1 block">Min ($/hr)</Label><Slider min={0} max={500} step={25} value={[filters.minRate]} onValueChange={([value]) => onFiltersChange({ ...filters, minRate: value })} /><div className="text-xs text-slate-400 mt-1">${filters.minRate}</div></div>
          <div><Label className="text-xs text-slate-500 mb-1 block">Max ($/hr)</Label><Slider min={50} max={1000} step={25} value={[filters.maxRate]} onValueChange={([value]) => onFiltersChange({ ...filters, maxRate: value })} /><div className="text-xs text-slate-400 mt-1">${filters.maxRate}</div></div>
        </div>
      </div>
      <div>
        ...
        <Slider min={1} max={5} step={1} value={[filters.minRating]} onValueChange={([value]) => onFiltersChange({ ...filters, minRating: value })} />
...
export default function AdvisorDirectory() {
  ...
  const queryFilters = useMemo(() => ({
    ...
    specialty: filters.specialties.length > 0 ? filters.specialties[0] : undefined,
  ...
  const { data, isLoading, isError } = useQuery<MarketplaceAdvisor[]>({
    queryKey: ['marketplace-directory', queryFilters],
    queryFn: async () => {
      const payload = await api.marketplace.search(queryFilters);
      const advisors = Array.isArray(payload)
        ? payload
      ...
      return advisors.map(normalizeMarketplaceAdvisor);
    },
    placeholderData: [],
  });
  ...
  const filtered = useMemo(() => advisors.filter((advisor) => {
    ...
    const name = String(advisor?.user?.fullName || '').toLowerCase();
    const bio = String(advisor?.bio || '').toLowerCase();
    const matchesSpecialty = Array.isArray(advisor.specialties) && advisor.specialties.some((specialty) => String(specialty).toLowerCase().includes(query));
    return name.includes(query) || bio.includes(query) || matchesSpecialty;
    ...
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex gap-3 mb-6">
          <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><Input placeholder="Search by name, specialty..." className="pl-10 bg-white" value={filters.search} onChange={(event) => { setFilters((prev) => ({ ...prev, search: event.target.value })); setPage(1); }} /></div>
          <Sheet>
            ...
            <SheetContent side="right" className="w-80 overflow-y-auto"><SheetHeader><SheetTitle>Filter Advisors</SheetTitle></SheetHeader><div className="mt-6"><FiltersPanel filters={filters} onFiltersChange={(value) => { setFilters(value); setPage(1); }} /></div></SheetContent>
          </Sheet>
        </div>
        <div className="flex gap-8">
          <aside className="hidden lg:block w-64 flex-shrink-0">
            <div className="bg-white rounded-xl border border-slate-200 p-5 sticky top-4">
              ...
              <FiltersPanel filters={filters} onFiltersChange={(value) => { setFilters(value); setPage(1); }} />
            </div>
          </aside>
          <div className="flex-1 min-w-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              ...
              ) : paged.map((advisor) => <AdvisorCard key={advisor.id} advisor={advisor} />)}
            </div>
            {!isLoading && totalPages > 1 && (
              <div className="mt-8 flex items-center justify-center gap-2">
                <Button variant="outline" size="sm" disabled={page === 1} onClick={() => setPage((current) => current - 1)}><ChevronLeft className="w-4 h-4" /></Button>
                {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
                  <Button key={pageNumber} variant={pageNumber === page ? 'default' : 'outline'} size="sm" className={cn(pageNumber === page && 'bg-indigo-600 hover:bg-indigo-700')} onClick={() => setPage(pageNumber)}>{pageNumber}</Button>
                ))}
                <Button variant="outline" size="sm" disabled={page === totalPages} onClick={() => setPage((current) => current + 1)}><ChevronRight className="w-4 h-4" /></Button>
              </div>
            )}
          </div>
        </div>
