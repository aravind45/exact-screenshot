      ...
      >
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={async () => {
              try {
                const { url } = await api.estates.getStatusLink();
                await navigator.clipboard.writeText(url);
                toast({
                  title: "Family update link copied",
                  description: "Text it to family — they can check progress anytime, no login needed.",
                });
              } catch {
                toast({
                  variant: "destructive",
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-indigo-50 text-indigo-600 text-[11px] font-bold border border-indigo-100 hover:bg-indigo-100 transition-colors"
            title="Copy a read-only progress link to share with family"
          >
            <Share2 className="w-3.5 h-3.5" /> Share family update
          </button>
