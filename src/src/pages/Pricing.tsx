...
export default function Pricing(): JSX.Element {
    ...
    const fetchLicenseClientSecret = useCallback(async () => {
        if (!user) {
            sessionStorage.setItem("after_login_redirect", "/pricing");
            navigate("/auth");
            return;
        ...
        if (typeof window !== 'undefined' && (window as any).gtag) {
            (window as any).gtag('event', 'begin_checkout', {
                currency: 'USD',
                value: 299,
                items: [{ item_id: 'estate_license', item_name: 'ExpectedEstate Estate License', price: 299 }],
            });
        }
        setLoading(true);
        try {
            const { clientSecret: secret } = await api.billing.createLicenseCheckout();
            setClientSecret(secret);
        } catch (error: any) {
            toast({
                variant: "destructive",
                ...
                <AnimatePresence mode="wait">
                    {!clientSecret ? (
                        ...
                        >
                            ...
                            {/* Pricing Cards */}
                            <div className="grid lg:grid-cols-2 gap-10 max-w-6xl w-full items-start">
                            {/* Estate License — one-time */}
                            <div className="relative group w-full">
                                <div className="absolute -inset-1 bg-gradient-to-r from-emerald-500 via-teal-600 to-emerald-600 rounded-[2.5rem] blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200" />

                                <Card className="relative bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden shadow-2xl h-full">
                                    <div className="p-10 flex flex-col h-full space-y-8">
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="text-[10px] font-black uppercase tracking-widest bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 px-3 py-1 rounded-full">
                                            ...
                                            <p className="text-slate-400 text-sm leading-relaxed">
                                                One payment covers this estate until it closes — even if settlement
                                                takes 18 months. No monthly bills during the 4-month creditor
                                                waiting period. Deductible as an estate expense.
                                            </p>
                                        </div>

                                        <div className="flex-1 space-y-4 text-slate-300">
                                            <FeatureItem icon={Check} text="Everything in the monthly plan, forever" />
                                            <FeatureItem icon={Check} text="Full Fiduciary OS for the life of the estate" />
                                            <FeatureItem icon={Check} text="Deadline engine, forms & accounting included" />
                                            <FeatureItem icon={Check} text="Never think about billing again" />
                                        </div>

                                        <div className="space-y-4">
                                            <Button
                                                size="lg"
                                                className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-lg py-7 rounded-2xl shadow-xl shadow-emerald-500/10 transition-all active:scale-95 disabled:opacity-50"
                                                onClick={fetchLicenseClientSecret}
                                                disabled={loading || !canCheckout}
                                            >
                                                {loading ? (
                                                    <Loader2 className="w-6 h-6 animate-spin" />
                                                ) : canCheckout ? (
                                                    "Buy Estate License — $299"
                                                ) : (
                                                    "Billing Unavailable"
                                                )}
                                            </Button>
                                            <p className="text-center text-xs text-slate-500 leading-relaxed px-2 pt-1">
                                                Prefer to try first? Start the 7-day free trial on the monthly
                                                plan, then switch — every feature is included either way.
                                            </p>
                                        </div>
                                    </div>
                                </Card>
                            ...
                            {/* Trust note */}
                            <div className="mt-16 text-center text-slate-500 font-medium max-w-lg">
                                <p className="mb-4 text-slate-400 text-sm leading-relaxed">
                                    Either plan includes every feature — the only difference is how you pay.
                                    Executors are personally liable for missed deadlines and accounting errors;
                                    both plans include the guardrails that protect you.
                                </p>
                                <p className="text-xs text-slate-600">
                                    Secure payments by Stripe · ExpectedEstate is self-help software, not a law firm
                                </p>
                            </div>
                        </motion.div>
                    ) : (
