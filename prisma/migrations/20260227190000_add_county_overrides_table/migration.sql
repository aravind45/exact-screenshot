-- Create county_overrides table for county-specific roadmap task overrides
-- This table allows customization of task titles, descriptions, fees, forms, and attachments at the county level.
...
CREATE TABLE IF NOT EXISTS public.county_overrides (
  ...
  CONSTRAINT county_overrides_state_code_county_name_task_id_key UNIQUE (state_code, county_name, task_id)
);

-- Create index for querying by state and county (idempotent)
CREATE INDEX IF NOT EXISTS idx_county_overrides_state_county ON public.county_overrides(state_code, county_name);

-- Create index for querying by task (idempotent)
