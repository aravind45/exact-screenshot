...
export const CommunicationService = {
    ...
    async getByAsset(assetId, estateId) {
    ...
    async getRecentByEstate(estateId, limit = 5) {
        return await prisma.communication.findMany({
            where: { estateId },
            take: limit,
            orderBy: { occurredAt: 'desc' },
    ...
    async delete(id, estateId) {
        return await prisma.$transaction(async (tx) => {
            const comm = await tx.communication.findUnique({ where: { id } });
            if (!comm || comm.estateId !== estateId)
    ...
    async getTimelineByEstate(estateId) {
        return await prisma.communication.findMany({
            where: { estateId },
            orderBy: { occurredAt: 'desc' },
