...
const DUPLICATE_TASK_PHASE_PREFERENCES = {
...
function harmonizeDuplicateTaskOwnership(phases, context = {}) {
    ...
    for (const [taskId, taskOccurrences] of occurrences.entries()) {
        ...
        const keep = preferredPhases
            ...
            .find((occ) => Boolean(occ)) || taskOccurrences[0];
        const removed = taskOccurrences.filter((occ) => !(occ.phaseIndex === keep.phaseIndex && occ.taskIndex === keep.taskIndex));
        if (removed.length === 0)
            continue;
        for (const entry of removed) {
            if (!removalsByPhase.has(entry.phaseIndex)) {
                removalsByPhase.set(entry.phaseIndex, new Set());
            }
    ...
    const harmonized = phases.map((phase, phaseIndex) => {
        ...
        if (!phaseRemovals || phaseRemovals.size === 0)
            return phase;
        return {
            ...phase,
