...
// ─── Jurisdictions ───
export async function listJurisdictions(status) {
...
export async function upsertJurisdiction(data, userId) {
    ...
    if (existing) {
      ...
      WHERE id = $1
    `, data.id, data.stateCode, data.stateName, data.fipsCode || null, data.timezone || null, data.isCommunityProperty || false, data.isUpcState || false, data.hasEstateTax || false, data.hasInheritanceTax || false, data.smallEstateThreshold || null, data.homesteadExemptionAmount || null, data.spousalElectiveSharePct || null, data.statuteOfLimitationsMonths || null, data.creditorClaimPeriodMonths || null, data.sourceCitation || null, userId || null);
...
export async function upsertProbateType(data, userId) {
    ...
    if (existing) {
      ...
      WHERE id=$1
    `, data.id, data.code, data.name, data.description || null, data.category || 'STANDARD', data.typicalDurationMonths || null, data.complexityTier || 2, data.requiresAttorney || false, data.requiresCourtHearing || true, data.requiresBond || false, data.sourceCitation || null);
  ...
  `, data.code, data.name, data.description || null, data.category || 'STANDARD', data.typicalDurationMonths || null, data.complexityTier || 2, data.requiresAttorney || false, data.requiresCourtHearing || true, data.requiresBond || false, data.sourceCitation || null);
    const created = rows[0];
    await logChange({ entityType: 'probate_type', entityId: created.id, action: 'CREATE', newValue: data, changedBy: userId });
    return created;
}
export async function deleteProbateType(id, userId) {
...
export async function listStatuteReferences(entityType, entityId) {
...
export async function upsertStatuteReference(data, userId) {
    if (data.id) {
        await executeSQL(`
      ...
      WHERE id=$1
    `, data.id, data.jurisdictionId || null, data.entityType, data.entityId, data.statuteCode, data.statuteTitle || null, data.statuteUrl || null, data.sectionNumber || null, data.effectiveDate || null, data.notes || null);
        return data;
    }
    const rows = await queryRows(`
  ...
  `, data.jurisdictionId || null, data.entityType, data.entityId, data.statuteCode, data.statuteTitle || null, data.statuteUrl || null, data.sectionNumber || null, data.effectiveDate || null, data.notes || null);
    return rows[0];
}
export async function deleteStatuteReference(id) {
    await executeSQL(`DELETE FROM ssot_statute_references WHERE id=$1`, id);
}
// ─── Publishing ───
export async function publishJurisdiction(id, userId) {
...
export async function publishProbateType(id, userId) {
