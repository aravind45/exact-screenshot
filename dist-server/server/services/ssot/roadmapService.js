...
// ─── Roadmaps ───
export async function listRoadmaps(jurisdictionId) {
...
export async function getRoadmapFull(id) {
...
export async function upsertRoadmap(data, userId) {
    if (data.id) {
        await executeSQL(`
      ...
      WHERE id=$1
    `, data.id, data.jurisdictionId, data.probateTypeId, data.name, data.description || null, data.estimatedDurationMonths || null, data.courtTypeId || null, data.filingAuthority || null, data.isDefault || false, data.sourceCitation || null, userId || null);
  ...
  `, data.jurisdictionId, data.probateTypeId, data.name, data.description || null, data.estimatedDurationMonths || null, data.courtTypeId || null, data.filingAuthority || null, data.isDefault || false, data.sourceCitation || null, userId || null);
    const c = rows[0];
...
export async function publishRoadmap(id, userId) {
...
// ─── Phases ───
export async function listPhases(roadmapId) {
    return queryRows(`
    SELECT p.*,
...
export async function upsertPhase(data, userId) {
    if (data.id) {
        await executeSQL(`
      ...
      WHERE id=$1
    `, data.id, data.roadmapId, data.code, data.title, data.subtitle || null, data.description || null, data.milestone || null, data.orderIndex, data.estimatedDays || null, data.isOptional || false, data.triggerCondition || null, data.sourceCitation || null);
  ...
  `, data.roadmapId, data.code, data.title, data.subtitle || null, data.description || null, data.milestone || null, data.orderIndex, data.estimatedDays || null, data.isOptional || false, data.triggerCondition || null, data.sourceCitation || null);
    const c = rows[0];
    await logChange({ entityType: 'phase', entityId: c.id, action: 'CREATE', newValue: data, changedBy: userId });
    return c;
}
export async function deletePhase(id, userId) {
...
// ─── Steps ───
export async function listSteps(phaseId) {
    return queryRows(`
    SELECT s.*,
...
export async function upsertStep(data, userId) {
    if (data.id) {
        await executeSQL(`
      ...
      WHERE id=$1
    `, data.id, data.phaseId, data.code, data.title, data.description || null, data.orderIndex, data.estimatedDays || null, data.isOptional || false, data.isConditional || false, data.conditionLogic ? JSON.stringify(data.conditionLogic) : null, data.responsibleParty || 'EXECUTOR', data.requiresAttorney || false, data.requiresCourtApproval || false, data.deadlineRule || null, data.deadlineDaysFromStart || null, data.riskWarning || null, data.rationale || null, data.sourceCitation || null);
...
// ─── Step Actions ───
export async function listStepActions(stepId) {
    return queryRows(`SELECT * FROM ssot_step_actions WHERE step_id=$1 ORDER BY order_index`, stepId);
}
export async function upsertStepAction(data, userId) {
    if (data.id) {
        await executeSQL(`
      ...
      WHERE id=$1
    `, data.id, data.stepId, data.code, data.title, data.description || null, data.orderIndex || 0, data.responsibleParty || 'EXECUTOR', data.deadlineRule || null, data.deadlineDays || null, data.isRequired !== false, data.isBlocking || false, data.actionType || 'TASK', data.outputArtifact || null, data.sourceCitation || null);
  ...
  `, data.stepId, data.code, data.title, data.description || null, data.orderIndex || 0, data.responsibleParty || 'EXECUTOR', data.deadlineRule || null, data.deadlineDays || null, data.isRequired !== false, data.isBlocking || false, data.actionType || 'TASK', data.outputArtifact || null, data.sourceCitation || null);
    const c = rows[0];
    await logChange({ entityType: 'step_action', entityId: c.id, action: 'CREATE', newValue: data, changedBy: userId });
    return c;
}
export async function deleteStepAction(id, userId) {
...
// ─── Step Dependencies ───
export async function listStepDependencies(stepId) {
    return queryRows(`
...
export async function addStepDependency(data) {
    const rows = await queryRows(`
...
export async function removeStepDependency(id) {
    await executeSQL(`DELETE FROM ssot_step_dependencies WHERE id=$1`, id);
}
// ─── Step Forms (linking) ───
export async function listStepForms(stepId) {
    return queryRows(`
...
export async function linkFormToStep(data) {
    const rows = await queryRows(`
    ...
    RETURNING *
  `, data.stepId, data.formId, data.isMandatory !== false, data.isConditional || false, data.conditionLogic ? JSON.stringify(data.conditionLogic) : null, data.orderIndex || 0, data.notes || null);
    return rows[0];
}
