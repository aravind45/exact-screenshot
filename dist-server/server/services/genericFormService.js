...
function resolveFieldValue(key, def, input, stateCode) {
...
function resolveComputed(key, input, stateCode) {
    ...
    switch (key) {
        ...
        case "totalRealProperty": {
            const realAssets = assets.filter((a) => a.inventoryCategory === 'ATTACHMENT_1' || a.category === 'real_property');
            return realAssets.reduce((sum, a) => sum + Number(a.inventoryValue || a.value || 0), 0);
        }
        case "totalPersonalProperty": {
            const personalAssets = assets.filter((a) => a.inventoryCategory !== 'ATTACHMENT_1' && a.category !== 'real_property');
 ...
 */
function getStateTerminology(stateCode) {
    const stateRule = getStateRule(stateCode);
    return {
 ...
 */
async function buildGenericPetition(fieldValues, stateCode, formType) {
 ...
 */
async function buildGenericInventory(fieldValues, stateCode) {
    const doc = await PDFDocument.create();
    const page = doc.addPage([612, 792]);
    const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
    const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
