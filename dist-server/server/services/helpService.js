...
export class HelpService {
     ...
     */
    static async processSupportMessage(userId, estateId, message, subject) {
        const user = await prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            throw new Error("User not found");
        // 1. Log Activity
        if (estateId) {
            await AuditService.logActivity(estateId, userId, 'SUPPORT_REQUESTED', 'SENT', `SUPPORT: Sent a message to support team regarding "${subject}".`);
        }
...
${message}
     ...
     */
    static async processFeedback(userId, rating, comment) {
        const user = await prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            throw new Error("User not found");
        const contactEmail = "expectedestate@gmail.com";
