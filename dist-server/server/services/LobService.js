...
const lob = new Lob(LOB_SECRET_KEY);
export const LobService = {
     ...
     */
    async sendLetter(estateId, recipient, fileBuffer, targetType, targetId) {
        try {
     ...
     */
    async syncMailingStatus(lobId) {
        try {
            const response = await lob.letters.retrieve(lobId);
            let status = 'PENDING';
            if (response.tracking_events?.some((e) => e.name === 'Delivered')) {
                status = 'DELIVERED';
            }
