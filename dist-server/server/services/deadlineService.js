...
const DEFAULT_STATE_RULE = { creditorClaimDays: 120, inventoryDays: 90, hasStateTax: false };
// ─────────────────────────────────────────────────────────────────────────────
...
export class DeadlineService {
    ...
    computeDeadlines(estate, state) {
        ...
        const pathType = (estate.authorityType || 'FORMAL_PROBATE');
        ...
        const push = (warningId, title, description, daysFromAnchor, anchorField, isStatutory = true) => {
            const anchor = this.getAnchorDate(estate, anchorField);
            if (!anchor)
                return; // skip if anchor date not yet recorded
        ...
        if (isTrust) {
            ...
            push('TRUSTEE_NOTICE_BENEFICIARIES', '⚠️ Trustee Notice to Beneficiaries Required', 'Notify all trust beneficiaries and heirs of your role as successor trustee, the existence of the trust, and their right to request a copy. Required by statute (e.g., CA Prob. Code §16061.7 = 60 days; most states 30–90 days).', 60, 'dateOfDeath');
            push('TRUST_ACCOUNTING_DUE', 'Trust Accounting to Beneficiaries', 'Provide a detailed accounting of all trust assets, income, expenses, and distributions to beneficiaries. Required annually and upon trust termination.', 365, 'dateOfDeath');
            push('TRUST_TAX_RETURN', 'Trust Income Tax Return (Form 1041) Due', 'File federal fiduciary income tax return (Form 1041) for the trust. Due by April 15 of the year following death, or 9 months from date of death — whichever comes first.', 270, 'dateOfDeath');
            if (isIrrevocable) {
                push('IRREVOCABLE_TRUST_EIN', 'Obtain Trust EIN from IRS', 'An irrevocable trust requires its own Employer Identification Number (EIN) immediately upon the grantor\'s death. Apply online at IRS.gov — typically same-day issuance.', 7, 'dateOfDeath');
            }
            push('FEDERAL_ESTATE_TAX_RETURN', 'Federal Estate Tax Return (Form 706) Due', 'File Form 706 if gross estate exceeds federal exemption (~$13.61M for 2024). Due 9 months from date of death; 6-month extension available.', 270, 'dateOfDeath');
            if (rules.hasStateTax) {
                push('STATE_ESTATE_TAX_RETURN', `${state} State Estate Tax Return Due`, `File ${state} state estate tax return. State exemptions are often lower than federal. Consult a CPA familiar with ${state} estate tax rules.`, 270, 'dateOfDeath');
            }
            // ── ALL OTHER (PROBATE) PATHS ────────────────────────────────────────
        }
        else {
            // 1. Creditor notice publication
            push('CREDITOR_NOTICE_PUBLICATION', isInsolvent
                ? '⚠️ CRITICAL: Publish Notice to Creditors (Insolvent Estate)'
                : 'Publish Notice to Creditors', isInsolvent
            ...
            push('FEDERAL_ESTATE_TAX_RETURN', 'Federal Estate Tax Return (Form 706) Due', 'File Form 706 if gross estate exceeds the federal exemption (~$13.61M for 2024). Due 9 months from date of death; automatic 6-month extension available with Form 4768.', 270, 'dateOfDeath');
            // 5. State estate tax (only for applicable states)
            if (rules.hasStateTax) {
                push('STATE_ESTATE_TAX_RETURN', `${state} State Estate Tax Return Due`, `File ${state} state estate tax return. Note: ${state}'s estate tax exemption is lower than the federal exemption — many mid-size estates owe state tax but not federal tax. Consult a local estate tax attorney.`, 270, 'dateOfDeath');
            }
            // ── INSOLVENT-SPECIFIC ─────────────────────────────────────────
            if (isInsolvent) {
                push('INSOLVENT_TAX_CLEARANCE', '⚠️ CRITICAL: Obtain Tax Clearance Before Any Distribution', 'Federal and state tax obligations are FIRST-PRIORITY debts in an insolvent estate. Obtain IRS tax clearance (estate closing letter or transcript) and any required state tax clearance BEFORE making any payments to unsecured creditors or heirs. Failure to do so may result in executor personal liability.', rules.creditorClaimDays + 30, 'noticePublishedDate');
                push('INSOLVENT_CREDITOR_PRIORITY', '⚠️ CRITICAL: Pay Creditors in Statutory Priority Order', 'Insolvent estate — assets must be distributed in this statutory order: (1) Funeral and administration expenses, (2) Federal taxes, (3) State taxes, (4) Secured creditors, (5) Unsecured creditors. Heirs receive NOTHING until all creditors are satisfied. Document every payment.', rules.creditorClaimDays + 45, 'noticePublishedDate');
            }
            // ── ANCILLARY-SPECIFIC ─────────────────────────────────────────
            if (isAncillary) {
                push('ANCILLARY_PROBATE_FILING', 'File Ancillary Probate in Secondary State', 'Out-of-state real property cannot be transferred using Letters from the primary state. File an ancillary (auxiliary) probate proceeding in the state where the out-of-state property is located. File within 60–90 days of receiving primary Letters to avoid delays.', 90, 'letterIssuedDate');
                push('ANCILLARY_CREDITOR_NOTICE', 'Secondary State: Publish Creditor Notice', 'After ancillary letters are issued, publish notice to creditors in the secondary state as required by that state\'s law. Creditor claim periods vary by state.', 120, 'letterIssuedDate');
                push('ANCILLARY_INVENTORY', 'Secondary State: File Ancillary Inventory', 'File an inventory of the out-of-state assets with the ancillary probate court within the time required by that state (typically 60–90 days after ancillary letters).', 150, 'letterIssuedDate');
            }
            // ── CONTESTED-SPECIFIC ─────────────────────────────────────────
            if (isContested) {
                push('CONTEST_RESPONSE_DEADLINE', '⚠️ CRITICAL: Respond to Will Contest — 30 Days', 'A will contest has been filed. You must file a formal written response with the probate court, typically within 30 days. ENGAGE A PROBATE LITIGATION ATTORNEY IMMEDIATELY. All asset distributions are SUSPENDED until the contest is fully resolved by the court.', 30, 'filingDate');
                push('CONTEST_DISCOVERY_PERIOD', 'Will Contest: Discovery Period', 'The court will set a discovery period (typically 120–180 days) during which both sides exchange evidence, take depositions, and prepare for hearing. No distributions may occur during this period.', 120, 'filingDate');
            }
        }
        return result;
    }
    ...
    getAnchorDate(estate, anchorField) {
        switch (anchorField) {
            case 'filingDate':
                ...
                return estate.courtFilingDate || estate.createdAt || null;
