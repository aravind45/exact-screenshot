...
const stateFormServices = new Map();
/**
...
export const StateFormServiceRegistry = {
     ...
     */
    async generateForm(stateCode, formId, input) {
     ...
     */
    resolveFields(stateCode, formId, input) {
        const service = this.getService(stateCode);
        const config = this.getConfig(stateCode);
     ...
     */
    getStatesWithDedicatedServices() {
        const states = [];
        for (const [stateCode, config] of stateFormServices) {
            if (!config.isGeneric) {
     ...
     */
    validateFormSupport(stateCode, formId) {
        const config = this.getConfig(stateCode);
 ...
 */
const statePriorityServices = new Map();
function initializePriorityRegistry() {
