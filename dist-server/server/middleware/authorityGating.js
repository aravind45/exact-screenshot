 ...
 */
export async function resolveEstateGates(estateId) {
    ...
    const authorityStatus = (estate.authorityStatus || "NOT_STARTED");
    ...
    for (const [gateType, definition] of Object.entries(GATE_DEFINITIONS)) {
        ...
        gates[gateType] = result;
        if (result.isOpen) {
            openGates.push(gateType);
        }
        else {
            closedGates.push(gateType);
        }
    }
 ...
 * Check a single gate
 */
function checkGate(currentStatus, authorityType, definition) {
 ...
 */
export async function checkOperationAllowed(estateId, operation) {
 ...
 */
export function requireFullAuthority(req, res, next) {
 ...
 */
export function requireAssetCollectionAuthority(req, res, next) {
    return requireAuthorityStatus({
 ...
 */
export function requireDistributionAuthority(req, res, next) {
    return requireAuthorityStatus({
 ...
 */
export function requireCreditorAuthority(req, res, next) {
    return requireAuthorityStatus({
 ...
 */
export async function getEstateGateStatus(estateId) {
    const resolution = await resolveEstateGates(estateId);
