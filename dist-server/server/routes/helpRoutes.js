...
router.post("/log", async (req, res) => {
    try {
        ...
        if (error instanceof z.ZodError)
            return res.status(400).json({ error: "Invalid help log request", details: error.errors });
...
router.post("/chat", async (req, res) => {
    try {
        ...
        if (error instanceof z.ZodError)
            return res.status(400).json({ error: "Question is required", details: error.errors });
...
router.post("/contact", async (req, res) => {
    try {
        ...
        if (error instanceof z.ZodError)
            return res.status(400).json({ error: "Invalid support request", details: error.errors });
