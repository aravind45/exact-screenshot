 ...
 */
router.get("/workflows/metrics", isAdmin, async (req, res) => {
    try {
        const now = new Date();
        const dayAgo = new Date(now.getTime() - (24 * 60 * 60 * 1000));
        const [inboxReceived, inboxFailed, inboxProcessing, inboxDeadLetter, inboxProcessed24h, inboxRetryDueNow, oldestInbox, outboxPending, outboxFailed, outboxProcessing, outboxDeadLetter, outboxProcessed24h, outboxRetryDueNow, oldestOutbox, workflowRunning, workflowFailed, workflowCompleted24h, deadLetterOpen, deadLetterReplayed24h, deadLetterBookingOpen, payoutReleaseEventsPending, payoutReleaseEventsDeadLetter, escrowHoldsActive, escrowReleasesDue,] = await Promise.all([
 ...
 */
router.get("/workflows/dead-letters", isAdmin, async (req, res) => {
    try {
 ...
 */
router.post("/workflows/dead-letters/:id/replay", isAdmin, async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.post("/workflows/drain", isAdmin, async (req, res) => {
    try {
