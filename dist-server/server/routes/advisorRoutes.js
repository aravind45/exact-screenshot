 ...
 */
router.post('/profile', authenticate, profileUpdateLimiter, async (req, res) => {
    try {
 ...
 */
router.post('/stripe/connect/onboard', authenticate, async (req, res) => {
    try {
 ...
 */
router.get('/dashboard/earnings', authenticate, async (req, res) => {
    try {
        const { prisma } = await import('../db.js');
        const advisor = await prisma.advisorProfile.findUnique({
            where: { userId: req.user.id }
