...
router.get("/asset/:assetId/document-recommendations", async (req, res) => {
    try {
...
router.get("/estate/available-documents", async (req, res) => {
    try {
        const estate = await prisma.estate.findFirst({ where: { userId: req.user.id } });
        if (!estate)
...
router.post("/validate-completeness", async (req, res) => {
    try {
        ...
        if (!asset)
            return res.status(403).json({ error: "Access denied" });
        const validation = await DocumentRecommendationService.validateCompleteness(assetId, attachedDocumentIds);
        res.json(validation);
    }
    catch (error) {
