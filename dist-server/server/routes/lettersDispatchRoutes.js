...
router.get("/my", async (req, res) => {
    try {
        ...
        if (!estate)
            return res.status(404).json({ error: "No estate found" });
 ...
 */
router.patch("/my/:id", async (req, res) => {
    try {
        const estate = await prisma.estate.findFirst({ where: { userId: req.user.id } });
        if (!estate)
            return res.status(404).json({ error: "No estate found" });
        ...
        if (certifiedCopyRef !== undefined)
            updateData.certifiedCopyRef = certifiedCopyRef;
        if (needsOriginal !== undefined)
            updateData.needsOriginal = needsOriginal;
        if (status !== undefined) {
 ...
 */
router.post("/my/reset", async (req, res) => {
    try {
        const estate = await prisma.estate.findFirst({ where: { userId: req.user.id } });
        if (!estate)
            return res.status(404).json({ error: "No estate found" });
 ...
 */
router.get("/my/pending-followups", async (req, res) => {
    try {
        const estate = await prisma.estate.findFirst({ where: { userId: req.user.id } });
        if (!estate)
            return res.status(404).json({ error: "No estate found" });
 ...
 */
router.post("/my/:id/generate-letter", async (req, res) => {
    try {
        ...
        if (!estate)
            return res.status(404).json({ error: "No estate found" });
        const dispatch = await prisma.lettersDispatch.findFirst({
            where: { id: req.params.id, estateId: estate.id },
        });
        if (!dispatch)
        ...
        const draw = (text, size = 11, font = fontRegular, xOff = 0) => {
            if (y < margin + 20) {
