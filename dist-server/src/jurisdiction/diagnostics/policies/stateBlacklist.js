 ...
 */
export function validateStateBlacklist(stateCode, _estateProfile, tasks) {
    ...
    for (const { term, description, severity } of config.expectedConcepts) {
        const termLower = term.toLowerCase();
        const foundInTasks = tasks.some(t => `${t.title} ${t.description || ''}`.toLowerCase().includes(termLower));
        if (!foundInTasks) {
            violations.push({
